...
public class ECGWaveView extends View {
    ...
    public ECGWaveView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void setLinePoint(List<Short> shortLists){
		if (null==shortLists) {
			return;
		}
		for (int i = 0; i < shortLists.size(); i++) {
			 mListPoint.add(new Point(i, (int) changeToScreenPosition(shortLists.get(i))));  
		}
		invalidate(); //刷新视图
	}
