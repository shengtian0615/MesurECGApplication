...
/**
 * AnimationEffect 安卓中为View添加动画效果
 */
public class AnimationEffect {
	...
	public static void AddAlphaAni(View view) {
		AddAlphaAni(view, 0, 1, 1000, Animation.REVERSE, Animation.RELATIVE_TO_PARENT);
	}

	/**
	 * 为View添加透明度变换效果，透明度从fromAlpha变化到toAlpha，变化持续时间durationMillis，
	 * 重复模式repeatMode
	 */
	public static void AddAlphaAni(View view, float fromAlpha, float toAlpha,
                                   long durationMillis, int repeatMode, int repeatCount) {
	 ...
	 * 设置位移动画效果，来回移动
	 */
	public static void setTransAni(View view, int x0, int x1, int y0, int y1,
                                   long durationMillis) {
	 ...
	 * 设置旋转动画
	 */
	public static void setRotateAni(View view, float fromDegrees,
                                    float toDegrees, long time) {
		setRotateAni(view, fromDegrees, toDegrees, time, Animation.REVERSE,
				Animation.INFINITE, true);
	}

	/**
	 * 设置旋转动画, 从角度fromDegrees旋转到toDegrees，旋转速度durationMillis毫秒，重启模式repeatModes，
	 * 重启次数repeatCount，是否匀速旋转linear。 示例：AnimationEffect.setRotateAni(view, 0,
	 * 360, 1000, Animation.RESTART, Animation.INFINITE, true); //
	 * 控制view每秒旋转360度
	 */
	public static void setRotateAni(View view, float fromDegrees,
                                    float toDegrees, long durationMillis, int repeatModes,
                                    int repeatCount, boolean linear) {
		RotateAnimation rotateAni = new RotateAnimation(fromDegrees, toDegrees,
	 ...
	 * 为控件添加尺寸渐变动画
	 */
	public static void setScaleAni(View V, float fromScale, float toScale,
                                   long ANITIME) {
		AnimationSet aniSet = new AnimationSet(true);
		// final int ANITIME = 500;

		// 尺寸变化动画，设置尺寸变化
		ScaleAnimation scaleAni = new ScaleAnimation(fromScale, toScale,
	...
	public static void setLightExpendAni(View V) {
		...
		scaleAni.setRepeatCount(Animation.RELATIVE_TO_PARENT);// 循环播放
		aniSet.addAnimation(scaleAni); // 将动画效果添加到动画集中
		...
		alphaAni.setRepeatCount(Animation.RELATIVE_TO_PARENT);// 循环播放
	...
	public static void setLightAni(final View view) {
		AnimationListener listenser = new AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				view.clearAnimation();
				setRotateAni(view, 36, 3996, 110000, Animation.RESTART,
						Animation.INFINITE, true);
			}
		...
		// 旋转
		RotateAnimation rotateAni = new RotateAnimation(0, 36,
				Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF,
				0.5f);
		...
		// 尺寸由小变大
		ScaleAnimation scaleAni = new ScaleAnimation(0.0f, 1.0f, 0.0f, 1.0f,
				Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF,
				0.5f);
