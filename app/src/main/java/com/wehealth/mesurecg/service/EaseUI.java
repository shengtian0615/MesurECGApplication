...
import com.wehealth.mesurecg.easemobmodel.EaseEmojicon;
...
public final class EaseUI {
	 ...
	 */
	private EMConnectionListener myConnectionListener = null;
...
//                	//处理系统消息
//                	if (message.getFrom().equals(Constant.APP_ADMIN)) {
...
//					}
                ...
                case EventNewCMDMessage: {
                    EMLog.e(TAG, "收到透传消息sdfgsdfg");
//                    EaseUI.getInstance().getNotifier().dealCmdMessage(message);
                    break;
                }
                default:
                    break;
	...
	private void initConnectListener() {
		myConnectionListener = new EMConnectionListener() {
			
			@Override
			public void onDisconnected(int error) {
				if (error == EMError.CONNECTION_CONFLICT) {
					try {
						if (AppManager.getAppManager().activityStack==null || AppManager.getAppManager().activityStack.isEmpty()) {
//							DoctorClinicApplication.getInstance().removePsd();
