...
public class RegexUtil {
	 ...
	 * 验证手机号码
	 * @param phoneStr 手机号码
	 * @return 如果是有效的返回true 手机不是必填项可以为空
	 */
	public static boolean phone(String phoneStr) {
		
		if ( phoneStr != null && (!"".equals(phoneStr)) && !phoneStr.matches(phoneReg)) {
			return false;
		} else
			return true;
	}
...
//          }
    ...
    //验证最后一位
    private boolean verifyLastNum(String eifhteencard) {
		String num = eifhteencard.substring(17, 18);
		char[] last = num.toCharArray();
		if (last[0]=='X' || (last[0] >= '0' && last[0] <= '9')) {
			return true;
		}
		return false;
	}
